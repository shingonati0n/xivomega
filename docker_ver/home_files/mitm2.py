...
class NullFirehoseException(Exception):
    pass
...
class Connection:
    ...
    def __init__(self, sock: socket.socket, source: typing.Tuple[str, int],
        ...
        logging.info(str(srv_port) + " " + str(srv_ip))
        ...
        logging.info("Destination IP: " + str(self.destination))
        for definition in definitions:
            for iprange in definition.Server_IpRange:
                if isinstance(iprange, ipaddress.IPv4Network):
    ...
    try:
        with open(cleanup_filename, "w") as fp:
            ...
            for i, rule in enumerate(load_rules(port, definitions, args.nftables)):
                if args.nftables:
                    add_cmd = f"nft rule ip nat PREROUTING {rule} dnat 14.14.14.2:{port} comment XMLM_{os.getpid()}_{i}_"
                    #add_cmd = f"nft rule ip nat PREROUTING {rule} dnat 192.168.100.82:{port} comment XMLM_{os.getpid()}_{i}_"
                else:
                    #add_cmd = f"iptables -t nat -I OUTPUT {rule} -j REDIRECT --to {port}"
                    add_cmd = f"iptables -t nat -I PREROUTING {rule} -j DNAT --to-destination 14.14.14.2:{port}"
                ...
                if args.nftables:
                    ...
                    #remove_cmd = f"iptables -t nat -D OUTPUT {rule} -j REDIRECT --to {port}"
                    remove_cmd = f"iptables -t nat -D PREROUTING {rule} -j DNAT --to-destination 14.14.14.2:{port}"
        ...
        #                    .read().strip().replace(" ", ""))
        os.system("echo skipping sysctl.ipv4.ip_forward as t comes with the pod")

        #removal_cmds.append("sysctl -w " + os.popen("sysctl net.ipv4.conf.all.route_localnet")
        #                    .read().strip().replace(" ", ""))
        os.system("echo skipping ipv4.conf.all.route_localnet too")
