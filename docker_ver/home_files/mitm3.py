...
def __main__() -> int:
    ...
    while True:
        ...
        try:
            ...
            #changing to localhost nust to check
    ...
    try:
            ...
            for i, rule in enumerate(load_rules(port, definitions, args.nftables)):
                ...
                if args.nftables:
                   ...
                   # remove_cmd = f"iptables -t nat -D PREROUTING {rule} -j REDIRECT --to {port}"
                    remove_cmd = f"iptables -t nat -D PREROUTING {rule} -j DNAT --to-destionation 14.14.14.2:{port}"
        ...
        #                    .read().strip().replace(" ", ""))
        os.system("echo skipping forward as that comes with the pod")

        #removal_cmds.append("sysctl -w " + os.popen("sysctl net.ipv4.conf.all.route_localnet")
        #                    .read().strip().replace(" ", ""))
        os.system("echo skipping route_localnet too")
